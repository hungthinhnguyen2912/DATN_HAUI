...
DATA_DIR = r"D:\\datn_haui\\mobilenetv2\\dataset\\fruits-360\\test"
TFRECORD_FILE = r"tfrecord_dataset\\test_3.tfrecord"
IMG_SIZE = (224, 224)
AUGMENT_SIZE = math.ceil(150 * 19 / 100)
SELECT_SIZE = math.ceil(300 * 19 / 100)
...
def process_and_save_tfrecord():
    ...
    with tqdm(total=total_images, desc="Processing Images", unit="img", colour="green", position=0, leave=True) as pbar:
        for label, class_name in enumerate(os.listdir(DATA_DIR)):
            ...
            if len(images) > (600 * 19 / 100):
                print(f'Class {class_name} có {len(images)} ảnh, chọn ngẫu nhiên ? ảnh')
                images = random.sample(images, SELECT_SIZE)

            elif len(images) < ( 100 * 19 / 100):
                print(f"Class {class_name} có {len(images)} ảnh, augment thêm ? ảnh")
                augmented_images = []
